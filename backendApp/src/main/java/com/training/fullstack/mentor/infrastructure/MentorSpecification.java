...
public class MentorSpecification  {
    ...
    public static Specification<Mentor> isAvailableOnTime(LocalDate desiredDate, LocalTime fromTime, LocalTime toTime) {
        return (root, query, cb) ->{
            SetJoin<Mentor, Calendar> mentorCalendar = root.joinSet("calendars", JoinType.INNER);
            Predicate startsBeforeTime = cb.lessThanOrEqualTo(mentorCalendar.get("startTime"),  fromTime);
            Predicate endsAfterTime = cb.greaterThanOrEqualTo(mentorCalendar.get("endTime"),  toTime);
            Predicate startsBeforeToday = cb.lessThanOrEqualTo(mentorCalendar.get("startDate"),  desiredDate);
            Predicate endsAfterToday = cb.greaterThanOrEqualTo(mentorCalendar.get("endDate"),  desiredDate);
            return cb.and(startsBeforeTime,endsAfterTime, startsBeforeToday, endsAfterToday);
        };
    ...
    public static Specification<Mentor> hasSkillId(Long skillId) {
        return (root, query, cb) -> {
            SetJoin<Mentor, MentorSkill> mentorSkills = root.joinSet("mentorSkills", JoinType.INNER);
            Join<MentorSkill, Skill> skills = mentorSkills.join("skill", JoinType.INNER);
            return cb.equal(
                    cb.lower(skills.get("id")),
                    skillId
            );
        };
    }
    ...
    public static Specification<Mentor> hasSkillIdAndIsAvailableOnTime(Long skillId,  LocalDate desiredDate, LocalTime fromTime, LocalTime toTime) {
        return hasSkillId(skillId).and(isAvailableOnTime(desiredDate,fromTime,toTime));
    }
}