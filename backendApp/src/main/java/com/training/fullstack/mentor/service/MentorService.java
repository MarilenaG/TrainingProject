...
public class MentorService  {
    ...
    public List<Mentor> searchMentorByTimeAndTechnologyAndDate (Long skillId, LocalDate desiredDate, LocalTime fromTime, LocalTime toTime){
        if (skillId!=null && desiredDate!= null){
            return  mentorRepository.findAll(MentorSpecification.hasSkillIdAndIsAvailableOnTime(skillId, desiredDate, fromTime, toTime));
        } else {
            if (skillId!=null){
                return  mentorRepository.findAll(MentorSpecification.hasSkillId(skillId));
            } else {
                return mentorRepository.findAll(MentorSpecification.isAvailableOnTime(desiredDate, fromTime,toTime));
            }
        }
    }
    ...
    public Mentor get(Long mentorId){
        return mentorRepository.findById(mentorId).orElseThrow(()-> new NoSuchElementException("No mentor with id "+ mentorId));
    }
